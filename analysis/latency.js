...
function groupByDay(data) {
    ...
    for (const { timestamp, latency } of data) {
        const day = Math.floor((now - timestamp.getTime()) / (24 * 60 * 60 * 1000));
        if (!buckets.has(day)) buckets.set(day, []);
        buckets.get(day).push(latency);
    }
    return buckets;
}
...
function pad(str, len, right = false) {
    return right ? String(str).padEnd(len) : String(str).padStart(len);
}
...
async function main() {
    ...
    for await (const line of rl) {
        ...
        if (timestamp && latency) data.push({ timestamp, latency });
    }
    if (data.length === 0) {
    ...
    for (const dayIndex of [...days.keys()].sort((a, b) => a - b)) {
        ...
        console.log(
            `${pad(label, 17, true)} │ ` +
                `${pad(s.n, 6)} │ ` +
                `${pad(s.p50.toFixed(1), 6)} │ ` +
                `${pad(s.p95.toFixed(1), 6)} │ ` +
                `${pad(s.p99.toFixed(1), 6)} │ ` +
    ...
    console.log(
        `${pad('OVERALL', 17, true)} │ ` +
            `${pad(overall.n, 6)} │ ` +
            `${pad(overall.p50.toFixed(1), 6)} │ ` +
            `${pad(overall.p95.toFixed(1), 6)} │ ` +
            `${pad(overall.p99.toFixed(1), 6)} │ ` +
    ...
    for (let hour = 0; hour < 24; hour++) {
        ...
        if (!values?.length) console.log(`${pad(hour.toString().padStart(2, '0') + ':00', 5, true)} │      - │      - │      - │      - │      - │      - │`);
        else {
